function varargout = NIRS_TaskAnalysis(varargin)
% NIRS_TASKANALYSIS MATLAB code for NIRS_TaskAnalysis.fig
%      NIRS_TASKANALYSIS, by itself, creates a new NIRS_TASKANALYSIS or raises the existing
%      singleton*.
%
%      H = NIRS_TASKANALYSIS returns the handle to a new NIRS_TASKANALYSIS or the handle to
%      the existing singleton*.
%
%      NIRS_TASKANALYSIS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NIRS_TASKANALYSIS.M with the given input arguments.
%
%      NIRS_TASKANALYSIS('Property','Value',...) creates a new NIRS_TASKANALYSIS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before NIRS_TaskAnalysis_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to NIRS_TaskAnalysis_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help NIRS_TaskAnalysis

% Last Modified by GUIDE v2.5 23-Nov-2019 14:09:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @NIRS_TaskAnalysis_OpeningFcn, ...
    'gui_OutputFcn',  @NIRS_TaskAnalysis_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before NIRS_TaskAnalysis is made visible.
function NIRS_TaskAnalysis_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to NIRS_TaskAnalysis (see VARARGIN)
...
% UIWAIT makes NIRS_TaskAnalysis wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = NIRS_TaskAnalysis_OutputFcn(hObject, eventdata, handles)
...
if ischar(inpath)
    ...
    if ~isempty(files)
        ...
        load(fullfile(inpath,fileList{1}));
        if exist('GLM_data','var')
            handles.inputtype=2;
            ...
            des_mtx=GLM_data.desmtx;
            des_mtx(:,end+1)=1;
            
            [m,n]=size(des_mtx);
            for i=1:n-1
                xlable{i}=['con',num2str(i),' '];
            end
...
if handles.inputtype==1 && ~isempty(inpath) && ~isempty(des_mat_path)
    ...
    handles.condi_num=condi_num;
    handles.desplot=1;
elseif handles.inputtype==2 && ~isempty(inpath)
...
% --- Executes on button press in averge.
function averge_Callback(hObject, eventdata, handles)
% hObject    handle to averge (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
NIRS_stat_average;
...
if ischar(fname)
    ...
    n_sublist=get(handles.fileList,'string');
    if n_designmat-1==length(n_sublist)
        ...
        msgbox({'The length of your design_inf.mat is unequal to the sub num!!!';'Please check !!!'},'Design inf is wrong!','warn');
    end
end
% Update handles structure
guidata(hObject, handles);
...
function contrast=inputcontrast(contr_num,condi_num)
...
elseif ~isempty(contrinf{1}) && ~isempty(contrinf{2})
    ...
    if isempty(strfind(contrast{2},']'))
        ...
        if length(contr_value)> condi_num+1
        ...
        elseif length(contr_value) == condi_num+1 && contr_value(end) ~= 0
